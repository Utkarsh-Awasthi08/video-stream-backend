package com.stream.spring_boot_app.controller;

import com.stream.spring_boot_app.AppConstants;
import com.stream.spring_boot_app.entities.Video;
import com.stream.spring_boot_app.payload.CustomMessage;
...
public class FileController {
    ...
    public FileController(VideoService videoService) {
    ...
    public ResponseEntity<?> process(@PathVariable String videoId) {
        try {
            boolean isProcessed = videoService.processVideo(videoId);
            if(!isProcessed) {
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Video processing failed");
            }
        } catch (Exception e) {
    ...
    public ResponseEntity<?> deleteVideo(@PathVariable("videoId") String videoId) {
        try {
    ...
    public ResponseEntity<?> cancelUpload(@PathVariable String id) {
        videoService.cancelUpload(id);
...
//    http://localhost:8080/api/v1/videos/stream/35212512
    ...
    @GetMapping("/search")
    public ResponseEntity<List<Video>> searchVideos(@RequestParam("title") String query) {
        List<Video> results = videoService.searchByTitle(query);
